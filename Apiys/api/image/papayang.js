...
module.exports = function(app) {
    ...
    app.get("/image/papayang", async (req, res) => {
        try {
            const keyword = req.query.q || getRandomKeyword();

            const data = await pinterest(keyword);

            if (!data.status) throw new Error(data.error);
            if (!data.result.length) throw new Error("Tidak ada gambar ditemukan");

            const validImages = data.result.filter(
                v => v.img && v.img !== "data tidak terbaca"
            );

            if (!validImages.length) throw new Error("Tidak ada gambar valid");
