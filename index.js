...
app.use("/", express.static(path.join(__dirname, "page-database")));
app.use("/Apiys", express.static(path.join(__dirname, "Apiys")));

const openApiPath = path.join(__dirname, "./Apiys/openapi.json");
let openApi = {};
...
let totalRoutes = 0;
const apiFolder = path.join(__dirname, "./Apiys/api");
...
app.get("/", (req, res) => res.sendFile(path.join(__dirname, "page-database", "index.html")));
app.get("/docs", (req, res) => res.sendFile(path.join(__dirname, "page-database", "docs.html")));

app.use((req, res) => res.status(404).sendFile(path.join(__dirname, "page-database", "404.html")));

app.use((err, req, res, next) => {
    console.error(err.stack);
    sendNotification(`🚨 Server Error: ${err.message}`);
    res.status(500).sendFile(path.join(__dirname, "page-database", "500.html"));
});
